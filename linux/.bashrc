...
# `1;` is the bold attribute (`0;` was normal intensity), so only \u and \W are
# bold. The brackets and `$` carry no escape code at all, leaving them — and any
# typed input, after the reset — at normal weight.
...
# fd >= 8.1 reads ~/.config/fd/ignore on its own, so no alias is needed here
# (the WSL config aliased fd to pass --ignore-file because fd 7.x lacked that).
# Ubuntu still ships the binary as `fdfind`; ~/.local/bin/fd symlinks it so that
# Telescope's find_files picks it up — see the setup steps in the README.
